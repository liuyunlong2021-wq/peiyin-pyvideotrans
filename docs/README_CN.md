...
桌面 GUI 还支持面向短剧的整集上下文翻译和本地声音克隆：先校对中文字幕，再校对英文与“接上句”发言轮次，最后用本地 Qwen3-TTS 按完整发言段克隆。字幕仍逐句保留，适合人工修正个别翻译或拆分 `A → B → A` 的轮次。翻译和人物建议使用配置的韭菜盒子 API，模型固定为 `gemini-3.6-flash`；API Key 保存在本地配置中。
