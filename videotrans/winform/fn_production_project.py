...
from PySide6.QtWidgets import (
    QComboBox, QFileDialog, QFormLayout, QHeaderView, QHBoxLayout, QLabel, QLineEdit,
    QMainWindow, QMessageBox, QPushButton, QStackedWidget, QTableWidget, QTableWidgetItem,
    QVBoxLayout, QWidget,
)

from videotrans.configure import config
from videotrans.configure.config import ROOT_DIR, app_cfg, params, tr
from videotrans.util.help_ffmpeg import get_video_info
from videotrans.util.help_misc import show_error
from videotrans.util.production_project import (
    STAGES, begin_stage, episode_name, import_original_video, isolated_work_dir, publish_file,
    read_states, require_stage, run_subtitle_remover, scaffold_project, snapshot_files, update_stage,
    validate_media, validate_project, verify_snapshot,
)
...
class RemoveSubtitleThread(QThread):
    ...
    def __init__(self, project, episode, video, parent=None):
        super().__init__(parent)
        self.project, self.episode, self.video = Path(project), episode, Path(video)
    ...
    def run(self):
        work = None
        try:
            input_snapshot = snapshot_files((self.video,))
            work = isolated_work_dir(config.TEMP_DIR, self.project)
            begin_stage(self.project, self.episode, "去除硬字幕")
            output = work / "target/无字幕.mp4"
            info = get_video_info(self.video.as_posix())
            run_subtitle_remover(ROOT_DIR, self.video, output, int(info["width"]), int(info["height"]))
            if self._cancelled.is_set():
                raise InterruptedError("任务已取消，未发布任何正式产物")
            verify_snapshot(input_snapshot)
            validate_media(output, video=True)
            destination = self.project / ".raw/media/视频" / self.episode / "无字幕.mp4"
            publish_file(output, destination, project=self.project)
            update_stage(
                self.project, self.episode, "去除硬字幕", "已完成", invalidate_downstream=True)
            self.done.emit(destination.as_posix())
        except Exception as error:
            try:
                update_stage(self.project, self.episode, "去除硬字幕", "失败")
            except Exception:
                pass
            message = getattr(error, "stderr", None) or getattr(error, "stdout", None) or str(error)
            self.failed.emit(f"{message[-3000:]}\n\n{traceback.format_exc()}")
        finally:
            if work:
...
class ProductionProjectWindow(QMainWindow):
    def __init__(self, remove_only=False):
        super().__init__()
        self.remove_only = remove_only
        self.task = None
        self.setWindowTitle(tr("Remove hard subtitles tool") if remove_only else tr("Production project"))
        self.resize(760 if remove_only else 900, 320 if remove_only else 640)
        ...
        if remove_only:
            page, page_layout = QWidget(), QVBoxLayout()
            page.setLayout(page_layout)
            remove_box, video_row = QFormLayout(), QHBoxLayout()
            self.video, self.select_video = QLineEdit(), QPushButton(tr("Select a Video"))
            self.video.setReadOnly(True)
            video_row.addWidget(self.video, 1)
            video_row.addWidget(self.select_video)
            remove_box.addRow(tr("Original video"), video_row)
            page_layout.addLayout(remove_box)
            self.start = QPushButton(tr("Remove hard subtitles only"))
            page_layout.addWidget(self.start)
            page_layout.addStretch(1)
            self.stage_pages.addWidget(page)
            self.task_buttons.append(self.start)
        else:
            ...
            self.open_remover = QPushButton(tr("Remove hard subtitles tool"))
            add_page(self.open_remover)
            ...
            add_page(self.start_recognition, self.import_recognition, self.export_recognition)
        ...
        if remove_only:
            ...
            self.start.clicked.connect(self.start_removal)
        else:
            self.open_remover.clicked.connect(self.open_removal_tool)
    ...
    def _refresh_stage_buttons(self, states):
        ...
        clean_video = project / ".raw/media/视频" / episode / "无字幕.mp4" if project else None
        rules = (
            (1, self.start_recognition, status("去除硬字幕") == "已完成" and clean_video.is_file(),
             "请先在左侧完成去除硬字幕"),
            (2, self.start_translation, status("中文识别") == "已确认", "请先校对并确认中文识别"),
            (3, self.start_dubbing, status("剧情翻译与轮次") == "已确认", "请先校对并确认剧情翻译与轮次"),
            (4, self.confirm_subtitle, status("英文克隆配音") == "已完成", "请先完成英文克隆配音"),
            (5, self.compose, status("英文克隆配音") == "已完成" and
    ...
    def open_removal_tool(self):
        window = app_cfg.child_forms.get("fn_remove_subtitles")
        if not window:
    ...
    def select_source_video(self):
        ...
        if filename:
            self.video.setText(filename)
            params["last_opendir"] = str(Path(filename).parent)
            params.save()

    def start_removal(self):
        if not self.project.text() or not Path(self.video.text()).is_file():
            show_error(tr("Select a production project and original video"))
            return
        try:
            episode = self._selected_episode()
            scaffold_project(self.project.text(), episode)
            source = import_original_video(self.project.text(), episode, self.video.text())
            self.video.setText(source.as_posix())
        except (ValueError, OSError) as error:
            show_error(str(error))
            return
        self._set_task_buttons(True)
        self.message.setText(tr("Removing original hard subtitles"))
        self.task = RemoveSubtitleThread(self.project.text(), episode, source, self)
        self.task.done.connect(self.removal_done)
        self.task.failed.connect(self.removal_failed)
        self.task.start()
        self.refresh()
    ...
    def start_project_recognition(self):
        try:
            ...
            if not video.is_file():
                raise FileNotFoundError(f"请先在左侧完成去除硬字幕：{video}")
        except Exception as error:
            show_error(str(error))
            return
        self._set_task_buttons(True)
    ...
    def start_project_dubbing(self):
        try:
            ...
            video = project / ".raw/media/视频" / episode / "无字幕.mp4"
            if not video.is_file():
                raise FileNotFoundError(f"请先完成去字幕：{video}")
    ...
    def removal_done(self, output):
        self._set_task_buttons(False)
        self.message.setText(f"{tr('Completed')}: {output}")
        self.refresh()

    def removal_failed(self, message):
        self._set_task_buttons(False)
        self.message.setText(tr("Failed"))
        self.refresh()
        show_error(message)
